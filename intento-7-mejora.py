...
def login(tipo=None, nombre=None, clave=None, campus_opcion=None):
    if tipo is None:
    ...
    if tipo == "1":
        ...
        if nombre is None or clave is None:
            print("1. Registrarse")
...
def crear_campus():
    ...
    with open(CAMPUS_LIST_FILE, "a") as f:
        f.write(nuevo + "\n")
...
def agregar_dispositivo(campus_actual):
    ...
    while True:
        ...
        for i, s in enumerate(servicios_opciones, start=1):
            print(f"{i}. {s}")
...
def eliminar_dispositivos(campus_actual):
    ...
    for i, b in enumerate(dispositivos, start=1):
